...
# 🗄️ DATABASE SETUP
# ==========================================
async_db_client = AsyncIOMotorClient(MONGO_URL, serverSelectionTimeoutMS=5000)
...
settings_collection = async_db_client["MockTestDB"]["settings"]
...
def get_test():
    try:
        ...
        total_qs = sum(len(s.get("questions", [])) for s in q)
        return jsonify({
            "questions": q, 
            "time": int(d.get("time", 15)), 
            "total_qs": total_qs, 
            "max_marks": total_qs * float(d.get("pos", 2.0)),
            "pos": float(d.get("pos", 2.0)), 
            "neg": float(d.get("neg", 0.25))
        })
...
async def send_test_to_channel(test_data, cid, client):
    ...
    total_qs = sum(len(s.get("questions", [])) for s in test_data["questions"])
...
async def run_bulk_transfer(client, status_msg, dest_id, uid, tests):
    ...
    if failed:
        ...
        if uid in failed_transfers: 
            del failed_transfers[uid]
...
async def start_cmd(client, message):
    try:
        ...
        await message.reply_text(f"❌ **ERROR IN /start:**\n\n`{err}`")
...
async def list_saved_channels(client, message):
    ...
    if not channels: 
        return await message.reply_text("❌ डेटाबेस में कोई चैनल सेव नहीं है!")
...
async def list_bot_users(client, message):
    ...
    count = len(users)
    if count == 0: 
        return await message.reply_text("❌ कोई यूज़र नहीं मिला!")
...
async def list_tests_with_channels(client, message):
    ...
    if not tests: 
        return await message.reply_text("❌ डेटाबेस में कोई टेस्ट नहीं है!")
    ...
    if os.path.exists("tests_channels.txt"): 
...
async def attempts_report(client, message):
    ...
    if not results: 
        return await wait.edit_text("❌ अभी तक कोई डेटा मौजूद नहीं है!")
...
async def remove_channel_cmd(client, message: Message):
    channels = await channels_collection.find().to_list(length=None)
    if not channels: 
        return await message.reply_text("❌ डेटाबेस में कोई चैनल नहीं है!")
...
async def transfer_tests(client, message):
    channels = await channels_collection.find().to_list(length=None)
    if not channels: 
        return await message.reply_text("❌ पहले `/add_channel` से चैनल जोड़ें!")
    ...
    await message.reply_text("📤 **सोर्स चैनल चुनें:**", reply_markup=InlineKeyboardMarkup(kb))

@app.on_message(filters.command("resend") & filters.private)
async def resend_tests(client, message):
    if len(message.command) < 2: 
        return await message.reply_text("⚠️ **इस्तेमाल का तरीका:** `/resend ExamName`")
    ...
    if not tests: 
        return await wait_msg.edit_text("❌ कोई टेस्ट नहीं मिला!")
    await wait_msg.edit_text(f"✅ **{len(tests)}** टेस्ट मिल गए हैं। सीधे यहाँ भेज रहा हूँ...")
    for test in tests:
        tid = test["test_id"]
        total_qs = sum(len(s.get("questions", [])) for s in test["questions"])
...
async def ask_next(client, uid, step, edit=None):
    ...
    if edit: 
        await edit.edit_text(msg, reply_markup=kb)
    else: 
...
async def callbacks(client, q):
    ...
    try:
        if cmd[0] == "set":
            ...
            await q.message.edit_text("📥 **नया चैनल चुनें जहाँ ट्रांसफर करना है:**", reply_markup=InlineKeyboardMarkup(kb))
            ...
            if not tests: 
                return await status_msg.edit_text("❌ कोई टेस्ट नहीं मिले!")
            ...
            if target_uid in failed_transfers and failed_transfers[target_uid]["tests"]:
                ...
                status_msg = await q.message.edit_text(f"🔄 **{len(tests_to_retry)}** फेल टेस्ट दोबारा भेजने की कोशिश चालू है...")
            ...
            if cmd[1] == "pub":
                ...
                channels = await channels_collection.find().to_list(length=None)
                if not channels: 
                    return await q.message.edit_text("❌ पहले `/add_channel` से चैनल जोड़ें।")
                kb = InlineKeyboardMarkup([[InlineKeyboardButton(ch["name"], callback_data=f"pubto:{tid}:{ch['chat_id']}")] for ch in channels])
                await q.message.edit_text("📢 **चैनल चुनें जहाँ पब्लिश करना है:**", reply_markup=kb)
            elif cmd[1] == "can":
                await q.message.edit_text("❌ प्रक्रिया रद्द कर दी गई।")
            ...
            await q.message.edit_text("❓ **क्या आप पक्का पब्लिश करना चाहते हैं?**", reply_markup=kb)

        elif cmd[0] == "confirm":
            tid, cid = cmd[1], cmd[2]
            test = await tests_collection.find_one({"test_id": tid})
            if not test: 
                return await q.message.edit_text("❌ टेस्ट डेटाबेस में नहीं मिला।")
            ...
            await q.message.edit_text(f"🗑 **टेस्ट सफलतापूर्वक डिलीट कर दिया गया!**\nID: `{tid}`")
    except Exception as e:
        err = traceback.format_exc()
        await q.message.reply_text(f"❌ **CALLBACK ERROR:**\n\n`{err}`")
...
async def text_handler(client, message):
    ...
    try:
        if message.text.startswith("TEST_"):
            ...
            test = await tests_collection.find_one({"test_id": tid})
            if not test: 
                return await message.reply_text("❌ यह Test ID डेटाबेस में नहीं मिली।")
        ...
        if step == "ASK_CH_NAME":
            ...
            try:
                ...
                await message.reply_text("❌ गलत ID! सिर्फ नंबर भेजें:")
            return
        ...
        if step in steps:
            ...
            if next_idx < len(steps):
                ...
                if next_step == "ASK_PAID" and user_steps[uid]["data"]["type"] != "COACHING": 
                    next_step = "ASK_NO"
                user_steps[uid]["step"] = next_step
                await ask_next(client, uid, next_step)
    except Exception as e:
        err = traceback.format_exc()
        await message.reply_text(f"❌ **TEXT HANDLER ERROR:**\n\n`{err}`")

@app.on_message(filters.document & filters.private)
async def json_handler(client, message):
    uid = message.from_user.id
    if uid not in user_steps or user_steps[uid]["step"] != "ASK_JSON": 
        return
    ...
    try:
        ...
        msg = get_premium_design(user_steps[uid]["data"], sum(len(s.get("questions", [])) for s in q_data))
        ...
        await wait.delete()
        await message.reply_text(f"👀 **यह आपके टेस्ट का Preview है:**\n\n{msg}", reply_markup=kb)
        del user_steps[uid]
    except Exception as e:
        err = traceback.format_exc()
        await wait.edit_text(f"❌ **JSON Upload Error:**\n\n`{err}`")
    finally:
...
if __name__ == "__main__":
    ...
    print("🤖 Starting Pyrogram Telegram Bot...")
    app.run()
