...
from .upload_handlers import upload_handlers
...
class AdminHandlers:
        ...
        if message:
            ...
            await telegram_utils.send_message(
                ...
                (
                    ...
                    f"🆔 आपका Telegram ID: `{user.id}`\n"
                    f"👤 Username: @{user.username or '-'}\n\n"
                    "Render → Environment में इस ID को ADMIN_IDS में "
                    "या username को ADMIN_USERNAMES में add करें।"
                ),
                parse_mode="Markdown",
            )
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        "🗄 DATABASE CHANNEL",
                        callback_data="admin:database_channel",
                    ),
                    InlineKeyboardButton(
                        "💳 PAYMENT CHANNEL",
                ...
                [
                    InlineKeyboardButton(
                        "💎 PAID USER CHANNEL",
                        callback_data="admin:paid_user_channel",
                    ),
                    InlineKeyboardButton(
                        "⚙️ SETTINGS",
    ...
    async def stats(
        ...
        await self._edit_or_send(
            ...
            context,
            text,
            InlineKeyboardMarkup([[self.back_panel()]]),
        )
    ...
    async def price_delete_start(
        self,
        update: Update,
        context: ContextTypes.DEFAULT_TYPE,
    ):
        context.user_data["admin_action"] = "price_delete"
        prices = db.get_setting("prices", {}) or {}
        plans = "\n".join(f"• `{name}` — ₹{value}" for name, value in prices.items()) or "कोई plan नहीं है।"
        await self._ask(update, context, f"🗑 **DELETE PRICE PLAN**\n\n{plans}\n\nजिस plan को हटाना है उसका नाम भेजें।")
    ...
    async def forcejoin_add_start(
        ...
        await self._ask(
            ...
            context,
            (
                "📢 Channel add करने के लिए इनमें से कोई एक भेजें:\n\n"
                "• `@channelusername`\n"
                "• Channel ID जैसे `-1001234567890`\n"
                "• या channel से कोई message यहाँ forward करें।\n\n"
                "⚠️ Bot को channel में Admin रखना जरूरी है।"
            ),
        )

    async def forcejoin_remove_start(
        self,
        update: Update,
        context: ContextTypes.DEFAULT_TYPE,
    ):
        ...
        if not channels:
            return await self._edit_or_send(
                update, context,
                "📢 कोई Force Join channel configured नहीं है।",
                InlineKeyboardMarkup([[self.back_panel()]]),
            )

        context.user_data["admin_action"] = "forcejoin_remove"
        lines = ["🗑 **REMOVE FORCE JOIN CHANNEL**\n"]
        for i, ch in enumerate(channels, 1):
            lines.append(f"{i}. {ch.get('title') or ch.get('username') or ch.get('channel_id')} — `{ch.get('channel_id')}`")
        lines.append("\nNumber, @username या channel ID भेजें।")
        await self._ask(update, context, "\n".join(lines))
    ...
    async def set_channel_start(
        ...
        await self._ask(
            ...
            context,
            (
                "📢 Channel add करने के लिए @username, -100... ID या channel से forwarded message भेजें।\n\n⚠️ Bot को channel में Admin बनाएं।"
            ),
        )
    ...
    async def handle_admin_text(
        ...
        if action in ("trial_days", "trial_limit"):
            try:
                value = int(text)
                ...
                await telegram_utils.send_message(
                    ...
                    "❌ 1 से 3650 के बीच valid number भेजें।",
                )
                return True

            key = "trial_days" if action == "trial_days" else "trial_extract_limit"
            db.set_setting(key, value, user.id)
            await self._admin_success(update, context, f"✅ {key} = `{value}` set कर दिया गया।")
        ...
        if action == "price_delete":
            ...
            if key in prices:
                prices.pop(key, None)
                db.set_setting("prices", prices, user.id)
                await self._admin_success(update, context, f"✅ Price plan `{key}` deleted.")
                context.user_data.pop("admin_action", None)
            else:
                await telegram_utils.send_message(context.bot, message.chat_id, "❌ यह price plan नहीं मिला। `/admin` से Price Settings फिर खोलें।", parse_mode="Markdown")
            return True
        ...
        if action == "forcejoin_add":
            ...
            channel = await self._resolve_channel_input(
                context, message
            )
            ...
            if not channel:
                await telegram_utils.send_message(
                    context.bot, message.chat_id,
                    "❌ Channel resolve नहीं हुआ। @username, -100... ID या forwarded channel message भेजें।",
                )
                return True

            if not await telegram_utils.bot_is_channel_admin(context, channel.get("channel_id")):
                await telegram_utils.send_message(
                    context.bot, message.chat_id,
                    "❌ Bot इस channel में Admin नहीं है। पहले Bot को channel में Admin बनाएं, फिर दोबारा add करें।",
                )
                return True

            db.add_force_join_channel(channel, added_by=user.id)
            db.set_setting("force_join_enabled", True, user.id)
            await self._admin_success(
                update, context,
                f"✅ Force Join channel added.\n📢 {channel.get('title') or channel.get('username')}\n🆔 `{channel.get('channel_id')}`",
            )
            context.user_data.pop("admin_action", None)
            return True

        if action == "forcejoin_remove":
            removed = db.remove_force_join_channel(text)
            if removed:
                await self._admin_success(update, context, "✅ Force Join channel removed.")
                context.user_data.pop("admin_action", None)
            else:
                await telegram_utils.send_message(context.bot, message.chat_id, "❌ Channel नहीं मिला। Number, @username या ID सही भेजें।")
            return True
        ...
        if action.startswith(
            ...
            if not await telegram_utils.bot_is_channel_admin(context, channel.get("channel_id")):
                await telegram_utils.send_message(
                    context.bot, message.chat_id,
                    "❌ Bot इस channel में Admin नहीं है। पहले Bot को channel में Admin बनाएं।",
                )
                return True
    ...
    async def handle_callback(
        ...
        if data == "admin:trial_days":
            context.user_data["admin_action"] = "trial_days"
            return await self._ask(
                update,
                context,
                "⏱ Trial कितने दिन का रखना है?\n\nउदाहरण: `7`",
            )

        if data == "admin:trial_limit":
            context.user_data["admin_action"] = "trial_limit"
            return await self._ask(
                update,
                context,
                "📚 Trial में कितने extracts allowed हों?\n\nउदाहरण: `3`",
            )
        ...
        if data == "admin:price_delete":
            return await self.price_delete_start(update, context)
        ...
        if data == "admin:forcejoin_remove":
            return await self.forcejoin_remove_start(
                update,
                context,
            )
        ...
        if data == "admin:database_channel":
        ...
        if data == "admin:paid_user_channel":
        ...
        if data.startswith(
            ...
            channel_type = data.split(":")[-1]
            db.remove_channel(channel_type)
            return await self.channel_settings(
                update,
                context,
                channel_type,
                f"{channel_type.upper()} CHANNEL",
            )
        ...
        if data == "admin:upload":
            # Use the real upload pipeline instead of setting an unused
            # admin_action. This makes ADD TEST actually functional.
            return await upload_handlers.start_upload(
                update,
                context,
            )
    ...
    async def _resolve_channel_input(self, context, message):
        """Resolve a channel from forwarded message, @username or chat ID."""
        if not message:
            return None

        # 1) Forwarded channel post (new Bot API).
        origin = getattr(message, "forward_origin", None)
        origin_chat = getattr(origin, "chat", None) if origin else None

        # 2) Legacy forward field.
        if origin_chat is None:
            origin_chat = getattr(message, "forward_from_chat", None)

        value = None
        if origin_chat is not None:
            value = origin_chat.id
        else:
            value = (message.text or message.caption or "").strip()

            # Accept https://t.me/channel as well as @channel.
        ...
        try:
            ...
            logger.warning("Channel resolve failed for %r: %s", value, exc)
            return None

        chat_type = getattr(chat, "type", None)
        if chat_type not in ("channel", "supergroup"):
            logger.warning("Resolved chat is not channel/supergroup: %s", chat_type)
            return None
    ...
    async def _bot_is_channel_admin(self, context, channel_id) -> bool:
        """Strictly verify that this bot is an administrator in the channel."""
        try:
            ...
            logger.warning("Channel admin check failed for %s: %s", channel_id, exc)
            return False

    def back_panel(self):
        """Return the Admin Panel back button (single button)."""
    ...
    def back_row(self):
        """Return the Admin Panel back button as a valid keyboard row."""
        return [self.back_panel()]
