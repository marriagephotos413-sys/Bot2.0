...
class AdminHandlers:
    ...
    async def test_report(
        ...
        report = db.get_extract_report() or {}

        # Current database returns a summary dictionary. Keep support for
        # older list-style reports as well so this button never crashes.
        if isinstance(report, dict):
            text = (
                "📊 **TEST EXTRACT REPORT**\n\n"
                f"📚 Total Requests: **{report.get('total', 0):,}**\n"
                f"⏳ Queued/Running: **{report.get('queued', 0):,}**\n"
                f"✅ Completed: **{report.get('completed', 0):,}**\n"
                f"❌ Failed: **{report.get('failed', 0):,}**\n"
            )
        elif isinstance(report, list):
            text = "📊 **TEST EXTRACT REPORT**\n\n"
            if not report:
                text += "अभी कोई extraction data नहीं है।"
            else:
                for index, item in enumerate(report, start=1):
                    ...
                    text += (
                        ...
                        f"   🎯 {item.get('exam', '-')}\n"
                        f"   🚀 Extracts: **{item.get('extract_count', 0)}**\n\n"
                    )
        else:
    ...
    async def queue_status(
        ...
        text = (
            "⚡ **QUEUE STATUS**\n\n"
            f"📥 Queued: **{status.get('queued', status.get('total_queued', 0))}**\n"
            f"🚀 Priority Queue: **{status.get('priority_queued', 0)}**\n"
            f"👤 Normal Queue: **{status.get('normal_queued', 0)}**\n"
            f"⚙️ Processing: **{status.get('processing', status.get('running', 0))}**\n"
            f"🔄 Retrying: **{status.get('retrying', 0)}**\n"
            f"✅ Done: **{status.get('done', status.get('completed', 0))}**\n"
    ...
    async def handle_admin_text(
        ...
        if action == "forcejoin_add":
            ...
            channel = await telegram_utils.resolve_channel_from_message(
                context, message
            )
