...
class TelegramUtils:
        ...
        for channel in channels:
            ...
            if not invite_link:
                try:
                    ...
                    logger.warning("Could not create Force Join invite for %s", channel.get("channel_id"), exc_info=True)
    ...
    async def resolve_channel_from_message(
        self,
        context: ContextTypes.DEFAULT_TYPE,
        message: Message,
    ) -> Optional[Dict[str, Any]]:
        """Resolve a channel from a forwarded channel post or normal text/ID."""
        if not message:
            return None

        # Telegram Bot API 7+ exposes the source chat through forward_origin.
        origin = getattr(message, "forward_origin", None)
        origin_chat = getattr(origin, "chat", None) if origin else None
        if origin_chat is not None:
            return await self._channel_info_from_chat(context, origin_chat)

        # Older Bot API/PTB objects expose forwarded channel as forward_from_chat.
        legacy_chat = getattr(message, "forward_from_chat", None)
        if legacy_chat is not None:
            return await self._channel_info_from_chat(context, legacy_chat)

        text = (message.text or message.caption or "").strip()
        return await self.resolve_channel(context, text)

    async def _channel_info_from_chat(
        self,
        context: ContextTypes.DEFAULT_TYPE,
        chat: Any,
    ) -> Optional[Dict[str, Any]]:
        try:
            # Refresh the chat from Telegram so title/username are authoritative.
            ...
            if username:
                ...
                # Private channels/groups do not have a public @username.
                # Because the bot is required to be an admin, it can create
                # a usable invite link for the Force Join button.
                try:
                    invite = await context.bot.create_chat_invite_link(
                        chat_id=fresh.id,
                        name="Bot Force Join",
                    ...
                    logger.warning("Could not create invite link for %s", fresh.id, exc_info=True)

            return {
                "channel_id": fresh.id,
                "title": getattr(fresh, "title", "") or "",
                "username": f"@{username}" if username else "",
                "invite_link": invite_link,
                "type": getattr(fresh, "type", "channel"),
            }
        except TelegramError as exc:
            logger.warning("Unable to read channel chat: %s", exc)
            return None
    ...
    async def resolve_channel(
        ...
        try:
            ...
            if getattr(chat, "type", None) not in ("channel", "supergroup", None):
                logger.warning("Resolved chat is not a channel/group: %s", value)
                return None
            ...
            if chat.username:
                ...
                try:
                    invite = await context.bot.create_chat_invite_link(
                        chat_id=chat.id,
                        name="Bot Channel Join",
                    )
                    invite_link = getattr(invite, "invite_link", "") or ""
                except TelegramError:
                    logger.warning("Could not create private invite link for %s", chat.id, exc_info=True)
    ...
    async def send_database_backup(
        ...
        # Accept both a CallbackContext/Application context and a raw Bot.
    ...
    # ============================================================
    # COMPATIBILITY DATABASE/FILE HELPERS
    # ============================================================

    async def send_database_test(
        self,
        bot: Bot,
        html: bytes,
        filename: str = "tcs.html",
        caption: str = "",
    ) -> Optional[Message]:
        """Compatibility wrapper used by the extraction pipeline."""
        return await self.send_database_backup(
            bot, html, filename, caption
        )
    ...
    async def send_document_bytes(
        ...
        return await self.send_document(
            bot, chat_id, document, filename, caption=caption
        )
    ...
    async def log_user_activity(
        ...
        try:
            ...
            # Activity logging must never block the user's normal bot flow.
            logger.exception("User activity channel send failed: %s", channel_id)
    ...
    async def log_paid_user(
        ...
        try:
            ...
            logger.exception("Paid user channel send failed: %s", channel_id)
