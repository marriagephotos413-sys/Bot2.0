...
class ExtractHandlers:
    ...
    async def process_extract_job(
        ...
        # --------------------------------------------------------
        # Preferred source: GitHub when configured
        # --------------------------------------------------------

        if github_enabled and github_path:
            await self.progress(
                internal_job_id,
                20,
        ...
        # --------------------------------------------------------
        # Fallback source: Telegram Database Channel
        # --------------------------------------------------------

        if not html:
            backup = db.get_backup(test_id)

            file_id = (backup or {}).get("file_id")

            if not file_id:
                raise ValueError(
                    "TCS source नहीं मिला। GitHub configure करें "
                    "या Database Channel backup उपलब्ध कराएँ।"
                )

            await self.progress(
                internal_job_id,
                20,
                "📢 Database Channel backup से TCS HTML download हो रहा है...",
            )
            ...
            html = bytes(file_bytes).decode(
                "utf-8",
                errors="replace",
            )

        if not html:
            raise ValueError("TCS HTML खाली है।")
