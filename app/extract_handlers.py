...
class ExtractHandlers:
    ...
    async def process_extract_job(
        ...
        # GitHub is optional. The upload pipeline stores only Telegram
        # file references in MongoDB, so the Database Channel file can be
        # downloaded directly when GitHub is unavailable.
        backup = db.get_backup(test_id) or {}

        if not html:
            database_file_id = backup.get("database_file_id") or ""
            if database_file_id and queue_manager.bot:
                try:
                    await self.progress(
                        internal_job_id, 28,
                        "📢 Database Channel से original HTML download हो रहा है...",
                    )
                    tg_file = await queue_manager.bot.get_file(database_file_id)
                    data = await tg_file.download_as_bytearray()
                    html = bytes(data).decode("utf-8", errors="ignore")
                except Exception:
                    logger.exception("Database Channel file download failed.")

        # Backward compatibility with older deployments that stored HTML in
        # the backup collection. New uploads no longer store full HTML there.
        if not html:
            html = backup.get("html") or ""

        if not html:
            raise ValueError("TCS HTML नहीं मिला। GitHub या Database Channel backup उपलब्ध नहीं है।")
        ...
        # Database channel
        # --------------------------------------------------------
        # Uploads already place the original file in the database channel.
        # Do not create a duplicate on every extraction. Older tests without
        # a stored file reference still get a fresh backup.
        database_message = None

        if not backup.get("database_file_id"):
            try:
                database_message = await telegram_utils.send_database_test(
                    bot=bot,
                    html=html.encode("utf-8"),
                    filename=backup.get("filename") or "tcs.html",
                    caption=backup.get("caption") or (
                        "📚 TEST DATABASE\n\n"
                        f"🆔 Test ID: `{test_id}`\n"
                        f"📝 {test.get('title', '-')}\n"
                        f"🎯 {test.get('exam', '-')}\n"
                ...
                logger.exception("Database channel backup failed")
                database_message = None
