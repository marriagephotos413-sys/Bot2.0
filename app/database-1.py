...
class MongoDatabase:
    ...
    def add_force_join_channel(
        ...
        """Add or update a Force Join channel without creating duplicates."""
        channels = self.get_force_join_channels() or []
        channel_id = channel.get("channel_id") if isinstance(channel, dict) else channel
    ...
    def remove_force_join_channel(
        ...
        """Remove Force Join channel by number, ID or @username."""
        channels = self.get_force_join_channels() or []
        ...
        if index is None:
            ...
            for i, ch in enumerate(channels):
                ...
                username = str(ch.get("username", "")).lower().lstrip("@")
                if cid == target or username == normalized:
                    index = i
                    break
        ...
        channels.pop(index)
        self.set_force_join_channels(channels)
        return True
