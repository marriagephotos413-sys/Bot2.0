...
class MongoDatabase:
    ...
    def set_channel(
        ...
        """Save a channel setting. Accepts either a raw ID or a resolved channel dict."""
        if isinstance(channel_id, dict):
            info = dict(channel_id)
            real_id = info.get("channel_id")
            title = info.get("title", title) or title
            username = info.get("username", username) or username
...
def _add_force_join_channel(self, channel, added_by=None):
    """Add a force-join channel without creating duplicates."""
    item = dict(channel) if isinstance(channel, dict) else {"channel_id": channel}
    item.setdefault("enabled", True)
    item.setdefault("added_at", self.utc_now())
    if added_by is not None:
        item["added_by"] = added_by

    channels = self.get_force_join_channels()
    existing_ids = {str(x.get("channel_id")) for x in channels if isinstance(x, dict)}
    if str(item.get("channel_id")) not in existing_ids:
        channels.append(item)
    return self.set_force_join_channels(channels, added_by)


def _remove_force_join_channel(self, identifier):
    """Remove a force-join channel by numeric ID, username or list index."""
    channels = self.get_force_join_channels()
    if not channels:
        return False

    value = str(identifier or "").strip()
    target = value.lstrip("@").lower()
    ...
    for index, item in enumerate(channels, start=1):
        ...
        if target == channel_id or target == username or target == str(index):
            removed = True
            continue
        kept.append(item)

    if removed:
        self.set_force_join_channels(kept)
    return removed
...
MongoDatabase.remove_force_join_channel = _remove_force_join_channel
...
def _get_channel_setting(self, name):
    # Compatibility aliases used by older payment code.
    aliases = {
        "payment_verify_channel": "payment",
        "payment_verification_channel": "payment",
        "database_channel": "database",
        "user_channel": "user_activity",
        "paid_channel": "paid_user",
    }
    channel_type = aliases.get(name, name)
    return self.get_channel(channel_type) or self.get_setting(name)
...
def _approve_payment(self, payment_id, admin_id=None):
    result = self.payments.update_one(
        {"payment_id": payment_id, "status": {"$in": ["pending", "submitted"]}},
        {"$set": {
            "status": "approved",
            "reviewed_at": self.utc_now(),
            "reviewed_by": admin_id,
...
def _add_paid_user(self, user_id, plan, payment_id=None, amount=0):
    ...
    return self.activate_paid_user(
        user_id, str(plan), expiry, amount=float(amount or 0), payment_id=payment_id
    )

def _get_paid_users_compat(self, limit=50, skip=0, page=None, per_page=None):
    if page is not None:
        page = max(1, int(page))
        limit = int(per_page or limit or 50)
        skip = (page - 1) * limit
    return list(
        self.users.find({"paid": True})
        .sort("paid_at", DESCENDING)
        .skip(int(skip or 0))
        .limit(int(limit or 50))
    )

MongoDatabase.get_paid_users = _get_paid_users_compat
