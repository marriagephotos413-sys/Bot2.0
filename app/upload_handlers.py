    ...
    async def process_upload_job(
        ...
        try:
            if CONFIG.github_token and CONFIG.github_owner and CONFIG.github_repo:
                ...
                logger.warning("GitHub not configured; using MongoDB backup fallback.")
        except Exception:
            logger.exception("GitHub upload failed; continuing with database backup fallback.")
        ...
        db.backups.update_one(
            {"test_id": test_id},
            {"$set": {"test_id": test_id, "html": html, "github_path": github_path,
                      "github_url": github_url, "updated_at": db.utc_now()},
