...
class UploadHandlers:
        ...
        for index, item in enumerate(
            ...
            job_id = await queue_manager.add_job(
                ...
                payload={
                    ...
                    "status_message_id": status.message_id if status else None,
                },
                is_admin=True,
            )
    ...
    async def process_upload_job(
        ...
        metadata = self.parse_caption(caption)

        # Caption has priority. Fill only missing fields from the actual
        # Telegram filename so both filename-only and caption-based uploads
        # follow the same metadata structure.
        filename_metadata = self.parse_caption(filename)
        for key in (
            "title", "series", "category", "exam", "section",
            "subsection", "test_type", "year", "language", "shift",
        ):
        ...
        # --------------------------------------------------------
        # Preserve uploaded HTML exactly
        # --------------------------------------------------------

        is_html_upload = extension in ("html", "htm")

        await queue_manager.progress(
            job_id,
            65,
            "🧩 Test HTML तैयार हो रहा है..." if is_html_upload else "🧩 TCS HTML generate हो रहा है...",
        )

        if is_html_upload:
            # User already supplied a ready HTML test. Do NOT rewrite it.
            # The original bytes are sent to the database channel with the
            # original filename and original caption.
        ...
        if is_html_upload:
            ...
            github_path = (
                f"tests/{self.safe_path(metadata.get('category', 'OTHER'))}/"
                f"{self.safe_path(metadata.get('exam', 'OTHER'))}/"
                f"{self.safe_path(metadata.get('test_type', 'Other'))}/"
                f"{self.safe_path(metadata.get('year', 'Other'))}/"
                f"{self.safe_path(source_stem)}.html"
            )
        else:
            github_path = self.build_github_path(metadata)

        await queue_manager.progress(
            job_id,
            75,
            "☁️ GitHub पर Test HTML upload हो रहा है...",
        )
        ...
        try:
            if CONFIG.github_token and CONFIG.github_owner and CONFIG.github_repo:
                github_result = github.upload_test(
                    path=github_path,
                    content=html,
                    message=("Add test: " + metadata["title"]),
                )
                github_url = (
                    github_result.get("html_url")
                    or github_result.get("download_url")
                    or ""
                )
            else:
                logger.warning("GitHub not configured; database channel will remain the primary backup.")
        except Exception:
            logger.exception("GitHub upload failed; continuing with database channel backup.")
        ...
        await queue_manager.progress(
            job_id,
            90,
            "📢 Original Test file Database Channel में भेजा जा रहा है...",
        )

        # --------------------------------------------------------
        # Database Channel — ORIGINAL FILE
        # --------------------------------------------------------

        # For HTML uploads the exact original file + exact original caption
        # are preserved. For JSON/TXT we send the generated TCS HTML.
        database_bytes = file_bytes if is_html_upload else html.encode("utf-8")
        database_filename = filename if is_html_upload else "tcs.html"
        database_caption = caption if is_html_upload and caption.strip() else self.database_caption(
            test_id, metadata, len(questions), github_url
        )

        database_message = await telegram_utils.send_database_backup(
        ...
        if not database_message:
            raise RuntimeError(
                "Database channel configured नहीं है या file भेजी नहीं जा सकी।"
            )

        # Store only Telegram references/metadata in MongoDB — never the
        # complete HTML/JSON payload. This also gives extraction a fallback
        # source when GitHub is temporarily unavailable.
        database_file_id = ""
        try:
            database_file_id = database_message.document.file_id if database_message.document else ""
        except Exception:
            database_file_id = ""
        ...
        db.backups.update_one(
            {"test_id": test_id},
            {
                "$set": {
                    "test_id": test_id,
                    "channel_id": getattr(database_message.chat, "id", None),
                    "message_id": database_message.message_id,
                ...
                "$setOnInsert": {"created_at": db.utc_now()},
            },
            upsert=True,
        )
    ...
    def parse_caption(
        ...
        # Use the complete caption/filename text for auto-detection too.
        # This allows a file with no caption to still resolve SSC/RRB/exam
        # metadata from names such as "SSC CGL Previous Paper 2019.html".
        full_text = (
            result["series"]
            + " "
            + result["title"]
    ...
    def extract_assigned_json(
        ...
        """Extract a JSON/JS array or object assigned to a variable."""
        for name in names:
            patterns = [
                rf"(?:let|const|var)\s+{re.escape(name)}\s*=\s*",
            ...
            for pattern in patterns:
                ...
                raw = self.extract_balanced_value(source, start)
                try:
                    return json.loads(raw)
                except json.JSONDecodeError:
                    # Common JavaScript export variation: trailing commas.
    ...
    @staticmethod
    def extract_balanced_value(source: str, start: int) -> str:
        """Return a balanced JSON/JS object while ignoring quoted braces."""
        opener = source[start]
        ...
        raise ValueError("Assigned JSON data incomplete है।")
    ...
    def extract_embedded_json(
        ...
        # --------------------------------------------------------
        # RRB / Testbook Q_DATA + Q_MAP format
        # --------------------------------------------------------
        # Example: var Q_DATA = [...]; var Q_MAP = {...};
        # These files are common in the HTML tests you upload.
        # Preserve HTML inside question/options because many questions
        # contain images.
        try:
            q_data = self.extract_assigned_json(
                html,
                [
                    "Q_DATA", "questionsData", "qData",
            ...
            q_map = self.extract_assigned_json(
                html,
                [
                    "Q_MAP", "qMap", "QUESTION_MAP", "questionMap",
                ],
            ) or {}

            if isinstance(q_data, list) and q_data:
                normalized = []
                for index, item in enumerate(q_data, start=1):
                    ...
                    map_entry = q_map.get(key, {}) if isinstance(q_map, dict) else {}
                    ...
                    answer = item.get("cidx")
                    if answer is None:
                        answer = item.get("correctIndex")
                    if answer is None:
                        answer = map_entry.get("correct")
                    if answer is None:
                        for key_name in ("correct_answer", "answer_number", "correct_option"):
                            if item.get(key_name) is not None:
                                try:
                                    answer = int(item[key_name]) - 1
                                except Exception:
                                    pass
                                break
                    if answer is None:
                    ...
                    normalized.append({
                        ...
                        "explanation": str(
                            ...
                            or map_entry.get("sol")
                            or map_entry.get("solution")
                            or map_entry.get("explanation")
                            or ""
                        ),
                        "sectionIndex": item.get("si", map_entry.get("si")),
                        "pos": item.get("pos", map_entry.get("pos")),
                        "neg": item.get("neg", map_entry.get("neg")),
                    })
            ...
            logger.debug(
                "Q_DATA/Q_MAP parser did not match this HTML; trying other formats.",
                exc_info=True,
            )
