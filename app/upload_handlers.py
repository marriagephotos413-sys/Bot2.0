...
class UploadHandlers:
    ...
    async def process_upload_job(
        ...
        #
        # GitHub is NOT required anymore.
        # If GitHub credentials are configured, the HTML is also
        # published there. Otherwise the Database Channel becomes
        # the source-of-truth backup.
        # --------------------------------------------------------
        ...
        if github_enabled:
            ...
            await queue_manager.progress(
                job_id,
                75,
                "📢 GitHub disabled है — Database Channel backup use होगा...",
            )
        ...
        # Save the Telegram file_id so extraction can work without
        # GitHub credentials.
        if database_message and getattr(database_message, "document", None):
            try:
                database_channel = db.get_channel("database") or {}
                db.save_backup_record(
                    test_id=test_id,
                    channel_id=database_channel.get("channel_id"),
                    message_id=database_message.message_id,
                    github_path=github_path,
                    github_url=github_url,
                    file_id=database_message.document.file_id,
                )
            except Exception:
                logger.exception(
                    "Database Channel backup record save failed."
                )
