...
class Config:
    ...
    # Accept the normal ADMIN_IDS plus common singular aliases.
    # This prevents the bot from silently treating the owner as a normal user
    # when Render was configured with ADMIN_ID / OWNER_ID instead of ADMIN_IDS.
    _ADMIN_RAW = (
        os.getenv("ADMIN_IDS", "").strip()
        or os.getenv("ADMIN_ID", "").strip()
        or os.getenv("OWNER_ID", "").strip()
        or os.getenv("OWNER_IDS", "").strip()
    )

    ADMIN_IDS = [
        int(user_id)
        for user_id in _get_list(_ADMIN_RAW)
        if user_id.lstrip("-").isdigit()
    ]

    # Optional username-based admin access. Examples:
    # ADMIN_USERNAMES=myusername,anotheradmin
    # ADMIN_USERNAME=myusername
    _ADMIN_USERNAME_RAW = (
        os.getenv("ADMIN_USERNAMES", "").strip()
        or os.getenv("ADMIN_USERNAME", "").strip()
        or os.getenv("OWNER_USERNAME", "").strip()
    )
    ADMIN_USERNAMES = {
        value.strip().lstrip("@").lower()
        for value in _get_list(_ADMIN_USERNAME_RAW)
        if value.strip()
    }
    ...
    def is_admin(self, user) -> bool:
        """Return True when a Telegram user matches configured admin ID or username."""
        if not user:
            return False
        try:
            if int(user.id) in self.ADMIN_IDS:
                return True
        ...
        return bool(
            username
            and str(username).lstrip("@").lower() in self.ADMIN_USERNAMES
        )
...
def validate_config() -> None:
    ...
    required = {
        ...
        "ADMIN_USERNAMES": sorted(CONFIG.ADMIN_USERNAMES),
    }
